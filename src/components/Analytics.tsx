...
function Analytics() {
  ...
  const [deadlineChartData, setDeadlineChartData] = useState<
    { month: string; deadline: number }[]
  >([]);
  const [totalTimeChartData, setTotalTimeChartData] = useState<
    { High: number; Low: number; Medium: number; month: string }[]
  >([]);
  const [selectedYear, setSelectedYear] = useState(-1);
  const [yearsToCompare, setYearsToCompare] = useState<number[]>([]);
  const [selectedYearsToCompare, setSelectedYearsToCompare] = useState<
    number[]
  >([]);
   ...
   */
  const formatCreationChartData = (
    metrics: { month: number; taskCount: number }[],
    year: number
  ) => {
  ...
  const formatTotalTimeChartData = (fullData: ITotalTime[]) => {
    const updatedData = fullData.reduce(
      (acc: any[], { month, priority, totalcompletiontime }: ITotalTime) => {
        ...
        if (monthIndex === -1) {
          ...
          acc[monthIndex][priority] =
            (acc[monthIndex][priority] || 0) + timeInHours;
        }
        return acc;
      },
      []
   ...
   */
  const mutationGetTaskCreationByYear = useMutation<
    any,
    Error,
    { year: number }
  >({
      ...
      if (data.statusCode === 200) {
        ...
        setCreationChartData((prev) => {
          const filtered = prev.filter(
            (prevFormatedData) =>
              prevFormatedData.id !== formatedData.id &&
              selectedYearsToCompare.includes(Number(prevFormatedData.id))
          );
   ...
   */
  const mutationGetTaskDeadlineByYear = useMutation<
    any,
    Error,
    { year: number }
  >({
  ...
  const handleAddYearToCompare = () => {
    const distinctNumbers = selectedYearsToCompare.filter(
  ...
  const handleRemoveYearToCompare = (removedYear: number) => {
    const updatedYears = selectedYearsToCompare.filter(
            ...
            <div className="absolute left-0 top-0 w-full h-full overflow-x-scroll scrollbar-none flex space-x-[20%]">
              {years.length > 0 &&
                years.map((year, index) => (
                  ...
                  >
                    <div
                      className={`w-5 h-5 bg-white border-2 border-violet-500 rounded-full cursor-pointer ${
                        selectedYear === year
                          ? "!bg-violet-500"
                          : "hover:bg-violet-500"
                      }`}
      ...
      <div className="h-4/5 w-full flex flex-shrink-0 mb-10">
        ...
        <div className="ms-5 w-1/4 p-4 bg-white border rounded-lg shadow-lg h-fit">
          ...
          <div className="flex flex-col w-full">
            {yearsToCompare.length > 0 &&
              yearsToCompare.map((year, index) => (
                <div className="flex my-1" key={"yearToAdd" + index}>
                  <button
                    className={`px-4 truncate py-2 me-1 w-3/5 rounded ${
                      year === selectedYear ||
                      selectedYearsToCompare.includes(year)
                        ? "bg-purple-600 text-white"
                        : "bg-white text-black"
                    } hover:bg-purple-600 hover:text-white transition border`}
                    disabled={year === selectedYear}
                    onClick={() =>
                      setSelectedYearsToCompare((prev) => [...prev, year])
                    }
                    title={"" + year}
                  >
                    {selectedYear === year ? "Now" : year}
                  ...
                  {year === selectedYear ||
                  !selectedYearsToCompare.includes(year) ? null : (
                    <div
                    ...
                    >
                      ...
                      <span className="ms-2 mobile:hidden laptop:block">
                        Remove
                      </span>
                    </div>
                  )}
